...
const parse = require("slick/parser");

const factory = require("./factory");
const typeOf = require("../util/type-of");

const types = factory.types;
const syntax = factory.syntax;
const instanceOf = factory.instanceOf;

const BaseList = factory.BaseList;
...
function matchContextParts(cache, context, parts, index) {
  ...
  const string = partString(part);
  const uid = context.uid;
  const cacheContext = cache[uid] || (cache[uid] = {});

  let matched;

  const last = (parts.length === index);
  ...
  if (cacheContext[string]) {
    ...
    iterators[part.combinator](context, (node, key) => {
  ...
  const resultSet = new BaseList;

  matched.forEach(node => {
    if (isPrimitive(node)) return false;

    const results = matchContextParts(cache, node, parts, index);
...
function search(context, expression) {
  ...
  const expressions = parse(expression);

  let matched;
  let lastExpression, firstExpression;
  const cache = {};
...
function match(node, part, key) {
  ...
  if (attributes) for (i = 0; i < attributes.length; i++) {
    ...
    const operator = attribute.operator, escaped = attribute.escapedValue;
    name = attribute.name;
  ...
  // pseudo selectors
  const pseudoSelectors = part.pseudos;
...
var iterators = {
  ...
  " ": function(node, visitor) { // many children
    return node.traverse((value, key) => visitor.call(node, value, key), true);
  },

  "~>": function(node, visitor) { // same as many children, but avoid block statements.
    return node.traverse((value, key, skip) => {
      if (
  ...
  ">": function(node, visitor) { // very direct children
    return node.traverse((value, key) => visitor.call(node, value, key));
  },
  ...
  "!": function(node, visitor) { // wow, such parents
    ...
    while (parentNode = parentNode.parentNode) {
      const ancestor = parentNode.parentNode;
      const result = visitor.call(node, parentNode, ancestor && ancestor.indexOf(parentNode));
...
const searchMethod = cw(function(expression) {
  return search(this, expression);
});

const parentMethod = cw(function(expression) {
  let parentNode = this;
  while (parentNode = parentNode.parentNode) {
...
const scopeMethod = cw(function(expression) {
  let parentNode = this;
  while (parentNode = parentNode.parentNode) {
...
const matchesMethod = cw(function(expression) {
  const expressions = parse(expression);
  ...
  const results = search(this.root, expression);

  for (let i = 0; i < results.length; i++) if (this === results[i]) return true;
  return false;
});
